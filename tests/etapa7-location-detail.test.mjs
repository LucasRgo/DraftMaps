  ...
  try {
    return {
      ...
      workerModule: await import(
        pathToFileURL(path.join(compiled.outputDirectory, "worker/index.js"))
          .href
      ),
    };
  } catch (error) {
...
function getWorkerFetch(workerModule) {
  ...
  if (typeof workerModule.default?.default?.fetch === "function") {
    return workerModule.default.default.fetch.bind(workerModule.default.default);
  }
...
test("Etapa 7 findLocationById returns the first matching location and null for missing ids", async () => {
  ...
  try {
    ...
    assert.deepEqual(module.findLocationById(locations, "goiania-node-1"), locations[0]);
    assert.equal(module.findLocationById(locations, "goiania-node-999"), null);
